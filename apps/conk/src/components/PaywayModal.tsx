...
export function PaywayModal({ vessel, hookTitle, mode, hasSecurityQ, autofuel, onConfirm, onCancel, isPending }: Props) {
  ...
  return (
    <div data-testid="payway-modal" style={{position:'fixed',inset:0,zIndex:201,background:'rgba(1,6,8,0.93)',backdropFilter:'blur(16px)',display:'flex',alignItems:'center',justifyContent:'center',padding:'20px'}}>
      <div style={{width:'100%',maxWidth:'400px',background:'var(--surface)',border:'1px solid var(--border2)',borderRadius:'var(--radius-xl)',overflow:'hidden',boxShadow:'0 0 48px rgba(0,184,230,0.07)'}}>
        ...
        <div style={{padding:'18px 20px'}}>
          ...
          {step === 'preview' && (
            <div data-testid="payway-step-preview">
              ...
              <div style={{marginBottom:'16px'}}>
                {[['Payment','Required'],['Fuel',noFuel?'Insufficient':'Available'],['Security',hasSecurityQ?'Required after payment':'None'],['Autofuel',autofuel?'On — burns automatically':'Off — manual confirm']].map(([k,v]) => (
              ...
              {noFuel && (
                <div data-testid="payway-no-fuel" style={{padding:'12px',background:'var(--burn-dim)',border:'1px solid var(--burn-line)',borderRadius:'var(--radius-lg)',textAlign:'center',marginBottom:'12px'}}>
                  <div style={{fontFamily:'var(--font-display)',fontSize:'13px',fontWeight:600,color:'var(--burn)',marginBottom:'4px'}}>Insufficient Fuel</div>
                  <div style={{fontFamily:'var(--font-mono)',fontSize:'10px',color:'rgba(255,45,85,0.6)',lineHeight:1.7}}>This vessel cannot complete the crossing. Draw fuel to continue.</div>
                </div>
              )}

              <div style={{display:'flex',gap:'8px'}}>
                <button data-testid="payway-cancel-btn" onClick={onCancel}
                ...
                <button data-testid="payway-continue-btn" onClick={() => setStep('confirm')} disabled={noFuel}
          ...
          {step === 'confirm' && (
            <div data-testid="payway-step-confirm">
              ...
              <div style={{display:'flex',gap:'8px'}}>
                <button data-testid="payway-back-btn" onClick={() => setStep('preview')}
                  style={{flex:1,padding:'11px',background:'none',border:'1px solid var(--border2)',borderRadius:'var(--radius)',color:'var(--text-dim)',fontFamily:'var(--font-mono)',fontSize:'11px',cursor:'pointer'}}>
                  ← Back
                </button>
                <button data-testid="payway-confirm-btn" onClick={onConfirm} disabled={!!isPending}
                  ...
                  {isPending ? <><span className="spinner" style={{borderTopColor:'var(--text-inv)',borderColor:'rgba(0,0,0,0.2)'}}/>opening...</> : 'Open Signal'}
                </button>
              </div>
            </div>
          )}
