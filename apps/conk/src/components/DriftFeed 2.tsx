  ...
  return (
    <>
      ...
      <div style={{borderBottom:'1px solid var(--border)'}}>
          ...
          <div style={{flex:1,minWidth:0,display:'flex',flexDirection:'column',gap:'5px'}}>
            <div style={{display:'flex',alignItems:'center',gap:'5px',flexWrap:'wrap'}}>
              {isFuture ? (
                ...
                <span className={`badge badge-${modeCls}`}>{modeLabel}</span>
              )}
              ...
              {hasSecurityQ&&!isFuture&&(
                <span style={{display:'inline-flex',alignItems:'center',gap:'3px',padding:'2px 7px',borderRadius:'100px',fontFamily:'var(--font-mono)',fontSize:'8px',fontWeight:600,letterSpacing:'0.06em',background:'rgba(94,79,232,0.1)',border:'1px solid rgba(94,79,232,0.2)',color:'var(--sealed)'}}>
                  🔐 gated
                </span>
              )}
        ...
        {/* Expanded panel */}
        {step!=='idle' && !isFuture && (
          <div style={{padding:'0 16px 16px',background:'var(--surface)',animation:'revealBody 0.22s ease both'}}>
            ...
            {isUnlocked&&(
              <>
                <div className="cast-body-revealed" style={{marginTop:'6px',marginBottom:'12px'}}>
                  {(cast.body??'').split('\n\n').map((p,i)=>(
                ...
                {burnCount===null&&!showReturn&&(
                  <div style={{display:'flex',gap:'8px',flexWrap:'wrap',marginBottom:'4px'}}>
                    ...
                    {/* Store — saves to vessel, prevents auto-burn */}
                    {!isStoredByMe&&!stored&&(
                    ...
                    {(isStoredByMe||stored)&&(
                      <span style={{display:'flex',alignItems:'center',gap:'5px',padding:'6px 12px',fontFamily:'var(--font-mono)',fontSize:'10px',color:'var(--teal)',opacity:0.6}}>
                        ✓ stored
                      </span>
                    )}
                    ...
                    {/* Viewer burn */}
                    {vessel?.id&&(
                      <button onClick={handleViewerBurn}
