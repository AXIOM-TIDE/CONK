  ...
  return (
    <div className="shell">
      ...
      <div style={{display:'flex',alignItems:'center',gap:'10px',padding:'0 16px',height:'52px',background:'rgba(3,12,20,0.92)',backdropFilter:'blur(20px)',borderBottom:'1px solid var(--border)',flexShrink:0,zIndex:20}}>
        <button onClick={onBack} style={{display:'flex',alignItems:'center',gap:'6px',background:'none',border:'none',color:'var(--text-dim)',fontFamily:'var(--font-mono)',fontSize:'11px',cursor:'pointer',padding:0}}>
      ...
      <div style={{flex:1,overflowY:'auto',padding:'20px 16px',scrollbarWidth:'thin',scrollbarColor:'var(--border2) transparent'}}>
        ...
        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:'16px'}}>
          <div>
            ...
            <div style={{fontFamily:'var(--font-mono)',fontSize:'11px',color:atCap?'var(--burn)':'var(--text-dim)',marginTop:'2px'}}>
              {allVessels.length} / {MAX_VESSELS} {atCap && '· capacity reached'}
            </div>
          </div>
          {atCap ? (
            <div style={{padding:'5px 12px',background:'var(--burn-dim)',border:'1px solid var(--burn-line)',borderRadius:'var(--radius)',fontFamily:'var(--font-mono)',fontSize:'9px',color:'var(--burn)',letterSpacing:'0.04em'}}>
        ...
        {allVessels.map((v, i) => {
          ...
          return (
            <div key={v.id} style={{background:'var(--surface)',border:`1px solid ${isActive?'var(--border3)':'var(--border)'}`,borderRadius:'var(--radius-xl)',marginBottom:'12px',overflow:'hidden',boxShadow:isActive?'0 0 20px rgba(0,184,230,0.07)':'none',animation:`rowIn 0.2s ease ${i*60}ms both`}}>
              ...
              <div style={{padding:'16px'}}>
                ...
                <div style={{display:'flex',alignItems:'center',gap:'12px',marginBottom:'14px'}}>
                  ...
                  <div style={{flex:1}}>
                    ...
                    <DecayBadge expiresAt={v.expiresAt} size="sm"/>
                  </div>
