...
export function Onboarding() {
  ...
  const launch = async () => {
    ...
    setVessel({ id:`v_${Math.random().toString(36).slice(2,10)}`, tier, tempOrPerm:'perm', createdAt:now, lastCastAt:null, expiresAt:now+yr, fuel:0, fuelDrawing:true, autoBurn:true })
      ...
      <div style={{ width:'100%', maxWidth:'380px', display:'flex', flexDirection:'column', gap:'24px', zIndex:1, animation:'fadeUp 0.35s ease both' }}>
        ...
        {step === 'welcome' && (
          <div style={{ display:'flex', flexDirection:'column', gap:'16px' }}>
            ...
            <button data-testid="onboard-continue" className="btn btn-primary btn-full" onClick={() => setStep('harbor')}>Get started →</button>
        ...
        {step === 'harbor' && (
          <div style={{ display:'flex', flexDirection:'column', gap:'16px' }}>
            ...
            <div style={{ display:'flex', gap:'8px' }}>
              ...
              <button data-testid="onboard-continue" className="btn btn-primary" style={{ flex:1, height:'42px' }} onClick={() => setStep('vessel')}>Harbor ready → next</button>
            </div>
          </div>
        )}
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:'16px' }}>
            ...
            <div className="mode-cards">
              {([
                ...
                <button key={t} data-testid={`tier-${t}`} className={`mode-card ${tier === t ? 'active' : ''}`} onClick={() => setTier(t)}>
