...
export interface Vessel {
  ...
  expiresAt: number
  fuel: number
  fuelDrawing: boolean   // autofuel — draws from Harbor automatically
  autoBurn: boolean      // auto-burn after interaction unless stored
}
...
export interface Cast {
  ...
  expiresAt: number
  createdAt: number
  lastInteractionAt: number    // resets decay timer on read
  tideCount: number
  // Tide read tracking (for Lighthouse qualification)
  tideReads: number[]          // read count per tide period
  burned?: boolean             // owner wrecked — global destruction
  burnedBy?: string[]          // vesselIds that burned from their view
  storedBy?: string[]          // vesselIds that stored this signal
  vesselTier?: VesselTier
  vesselId?: string
  requiresDockId?: string
  securityQuestion?: string
  securityAnswer?: string
  keywords?: string[]          // searchable metadata, not shown to readers
  unlocksAt?: number           // future release — locked until this timestamp
}
...
const SEED_CASTS: Cast[] = [
  {
    ...
    body:'Every message you have ever sent has a return address. Your IP. Your account. Your device fingerprint.\n\nWe built this so that none of that exists. Not encrypted. Not hidden. Simply never built.',
  ...
  },
  {
    ...
    body:'Agent A sourced leads. Agent B wrote outreach. Agent C handled replies. Three vessels, one Harbor, zero cross-contamination.\n\nThis is what agentic infrastructure looks like when privacy is the default.',
  ...
  },
  {
    ...
    body:'Three casts crossed 100k reads in under 6 hours. None were news. None were political. All three were personal truths told anonymously.\n\nThe tide does not reward outrage. It rewards honesty.',
  ...
  },
  {
    ...
    body:'You found it. The Dock is open. You know what to do.',
  ...
  },
  {
    ...
    body:'Legal entity: formed. Bank account: opened. Contracts: signed. None of them know my name.\n\nPrivacy is not paranoia. It is infrastructure.',
  ...
  },
  {
    ...
    body:'You read it. It is already beginning to dissolve. Gone when you close this.',
  ...
  },
  {
    ...
    body:'You found it early. The tide rewards patience.',
    ...
    unlocksAt: NOW + 6*3600000, // unlocks in 6 hours
...
export const useStore = create<AppState>()(
  persist(
    (set, get) => ({
      ...
      setVessel: (v) => set((s) => ({ vessel: v, vessels: v ? syncVessel(v, s.vessels) : s.vessels })),
      ...
      markCastRead: (id, body) => set((s) => ({
        driftCasts: s.driftCasts.map(c => c.id === id ? {
          ...
          // Reset expiresAt based on last interaction
          expiresAt: Date.now() + castDurationMs(c.duration),
          tideReads: [c.tideReads[0] + 1, c.tideReads[1] ?? 0, c.tideReads[2] ?? 0],
        } : c),
      })),
