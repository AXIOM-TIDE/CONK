...
export function CastPanel({ onClose }: { onClose: () => void }) {
  ...
  if (step === 'confirm') return (
    <>
      ...
      <button className="btn btn-primary btn-full" onClick={handleSend} disabled={isSending||isDone||lowFuel}>
        {isSending ? <><span className="spinner"/>Sounding…</> : isDone ? '✓ cast sounded' : <><IconCast size={12} color="var(--text-inv)"/> Sound it · $0.001</>}
      </button>
  ...
  return (
    <>
      ...
      <div className="field" style={{marginBottom:'11px'}}>
        ...
        <textarea className="input" rows={2} placeholder="The line they see first..." value={hook} onChange={e=>setHook(e.target.value)} maxLength={160} autoFocus/>
      ...
      <button className="btn btn-primary btn-full" onClick={() => { if (!hook.trim()) return; setStep('confirm') }} disabled={!hook.trim()||lowFuel||(useSecQ&&(!secQ.trim()||!secA.trim()))}>
        Review →
      </button>
    </>
  )
